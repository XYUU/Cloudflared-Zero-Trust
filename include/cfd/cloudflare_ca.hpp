// SPDX-License-Identifier: Apache-2.0
//
// Cloudflare Origin CA certificates, embedded so that the QUIC edge connection
// can be verified without relying on the system CA store.
//
// Source: github.com/cloudflare/cloudflared/tlsconfig/cloudflare_ca.go
// The QUIC tunnel edge (region*.argotunnel.com:7844) presents a cert signed by
// "CloudFlare Origin SSL ECC Certificate Authority", which is NOT in standard
// public root CA stores.
#pragma once
#include <string_view>

namespace cfd {

// PEM bundle containing Cloudflare's Origin SSL CA certificates.
// Pass this to QuicConfig::ca_bundle_path (written to a temp file) or load
// directly into an OpenSSL X509_STORE via SSL_CTX_load_verify_locations.
inline constexpr std::string_view kCloudflareCAPem = R"(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)";

}  // namespace cfd
